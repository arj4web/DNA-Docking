...
__global__ void gpu_translate(struct Nucleic_Acid Nucleotide,float x_shift, float y_shift, float z_shift,int ydim)
{
    ...
    if(nucleotide<ydim){
        if((nucleotide>0)&&(atom>0)&&(atom<=Nucleotide[nucleotide].size)){
            Nucleotide[nucleotide].atom[atom].coord[1] += x_shift;
            Nucleotide[nucleotide].atom[atom].coord[2] += y_shift;
            Nucleotide[nucleotide].atom[atom].coord[3] += z_shift;
...
struct DNA_structure translate_dna_structure(struct DNA_structure dna, float x_shift, float y_shift, float z_shift){
    ...
    int a=0;
    nucleotide = (struct Nucleic_Acid*)malloc((dna.length+1)*sizeof(struct Nucleic_Acid));
    for (int i = 1; i <= dna.length; i++)
    {
        nucleotide[i]=dna.nucleotide[i];
        cudaMalloc((void**)&nucleotide[i].atom,(dna.nucleotide[i].size+1)*sizeof(struct Atom));
    ...
    for (int i = 1; i <= dna.length; i++)
    {
        ...
        cudaMemcpy(new_dna.nucleotide[i].atom,nucleotide[i].atom,(dna.nucleotide[i].size+1)*sizeof(struct Atom),cudaMemcpyDeviceToHost);
...
__global__ void RotateonGPU(Nucleic_Acid *nucleotide,int z_twist , int theta , int phi,int ydim )
{
  int Nucleotide=threadIdx.y+(blockDim.y*blockIdx.y);
  int Atom=threadIdx.x+(blockDim.x*blockIdx.x);
...
if(Nucleotide<ydim){
  if((Nucleotide>0)&&(Atom>0)&&(Atom<=nucleotide[Nucleotide].size)){
      ...
      post_z_twist_x = nucleotide[Nucleotide].atom[Atom].coord[1] * cos( 0.017453293 * z_twist ) - nucleotide[Nucleotide].atom[Atom].coord[2] * sin( 0.017453293 * z_twist ) ;
      post_z_twist_y = nucleotide[Nucleotide].atom[Atom].coord[1] * sin( 0.017453293 * z_twist ) + nucleotide[Nucleotide].atom[Atom].coord[2] * cos( 0.017453293 * z_twist ) ;
      post_z_twist_z = nucleotide[Nucleotide].atom[Atom].coord[3] ;
      ...
      nucleotide[Nucleotide].atom[Atom].coord[1] = post_theta_x * cos( 0.017453293 * phi ) - post_theta_y * sin( 0.017453293 * phi ) ;
      nucleotide[Nucleotide].atom[Atom].coord[2] = post_theta_x * sin( 0.017453293 * phi ) + post_theta_y * cos( 0.017453293 * phi ) ;
      nucleotide[Nucleotide].atom[Atom].coord[3] = post_theta_z ;
...
/************/
  ...
  dim3 numblocks((a/threadperblock2D.x)+1,(dna.length/threadperblock2D.y)+1);
  RotateonGPU<<<numblocks,threadperblock2D>>>(d_nucleotide,z_twist,phi,theta,dna.length+1);
  cudaDeviceSynchronize();
